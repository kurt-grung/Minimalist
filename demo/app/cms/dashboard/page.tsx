...
export default function AdminDashboard() {
  ...
  const loadSettings = async () => {
    try {
      const response = await fetch('/cms/api/settings')
      if (response.ok) {
  ...
  const handleDelete = async (slug: string) => {
    ...
    try {
      const token = localStorage.getItem('admin_token')
      const response = await fetch(`/cms/api/posts/${slug}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  return (
    <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '2rem' }}>
      <header style={{ 
        ...
        <div>
          <h1 style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>CMS Dashboard</h1>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing && slugChanged) {
        ...
        const createResponse = await fetch('/cms/api/posts', {
          method: 'POST',
          headers: {
        ...
        // Delete old post
        const deleteResponse = await fetch(`/cms/api/posts/${post.slug}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          }
        ...
        const url = isEditing ? `/cms/api/posts/${post.slug}` : '/cms/api/posts'
